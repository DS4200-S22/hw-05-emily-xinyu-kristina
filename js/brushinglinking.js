...
//TODO: Barchart with counts of different species
{
  const bar_counts = [
      ...
      // Find the maximum count in the data, store in the maxY3 variable
      let maxY3 = d3.max(bar_counts, function(d) { return d[yKey3]; });

      // Create the y scale with domain and range
      y3 = d3.scaleLinear()
              .domain([0,maxY3])
              .range([height-margin.bottom,margin.top]);

      // Create an scale offset for each bar
      x3 = d3.scaleBand()
              .domain(d3.range(bar_counts.length))
              .range([margin.left, width - margin.right])
              .padding(0.2);
      ...
      svg3.append("g")
          ...
          .call(d3.axisBottom(x3)
                  .tickFormat(i => bar_counts[i].Species))
          .attr("font-size", '20px')
          .call((g) => g.append("text")
              .attr("x", width - margin.right)
              .attr("y", margin.bottom - 4)
...
function clear() {
    ...
    //TODO: add code to clear existing brush from
